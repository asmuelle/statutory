...
const PROSE_DATE = /\b(January|February|March|April|May|June|July|August|September|October|November|December)\s+(\d{1,2}),\s+(\d{4})\b/g;
const ISO_DATE = /\b(\d{4})-(\d{2})-(\d{2})\b/g;
const EFFECTIVE_PROSE = /\beffective\s+(?:on\s+|date\s+of\s+|beginning\s+(?:on\s+)?)?(January|February|March|April|May|June|July|August|September|October|November|December)\s+(\d{1,2}),\s+(\d{4})\b/gi;
...
const dedupeSorted = (dates: readonly string[]): readonly string[] =>
  [...new Set(dates)].sort();

/** All dates mentioned in a text, as sorted unique ISO strings. */
export const extractAllDates = (text: string): readonly string[] => {
  const prose = [...text.matchAll(PROSE_DATE)].map((m) => toIso(m[1] ?? '', m[2] ?? '', m[3] ?? ''));
...
export const extractEffectiveDates = (text: string): readonly string[] =>
  dedupeSorted(
    [...text.matchAll(EFFECTIVE_PROSE)].map((m) => toIso(m[1] ?? '', m[2] ?? '', m[3] ?? '')),
  );
